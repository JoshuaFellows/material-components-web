...
test('#changeToolbarStyles will emit change event and modify header style', () => {
  const {foundation, mockAdapter} = setupTest();
...
test('#changeToolbarStyles will not execute if we scrolled out of theshold', () => {
  ...
  td.verify(foundation.calculateFlexibleExpansionRatio_(), {times: 0});
  mockRaf.restore();
});
...
test('#setKeyHeights sets the desktop row height and ratio correctly', () => {
  ...
  td.verify(mockAdapter.setStyleForFixedAdjustElement('marginTop', `${toolbarHeight}px`));
...
test('#setKeyHeights do not recalcute height if toolbar row height is not changing', () => {
  const {foundation, mockAdapter} = setupTest();
  const mockRaf = createMockRaf();

  const viewportWidth = 1024;
  td.when(mockAdapter.getViewportWidth()).thenReturn(viewportWidth);
  foundation.keyHeights.toolbarRowHeight = 64;
  foundation.setKeyHeights();
  mockRaf.flush();
...
test('#setKeyHeights do not recalcute ratio when it is not initializing', () => {
  const {foundation, mockAdapter} = setupTest();
  const mockRaf = createMockRaf();
  ...
  td.verify(mockAdapter.getFlexibleRowElementOffsetHeight(), {times: 0});
  td.verify(mockAdapter.getOffsetHeight(), {times: 0});
  mockRaf.restore();
});
...
test('#changeFlexibleRowElementStyles_ does not sets the height of flexible row when toolbar is not fixed', () => {
  const {foundation, mockAdapter} = setupTest();
  const flexibleExpansionRatio = 1;
...
test('#changeElementStylesDefaultBehavior_ does not transform title when toolbar is not fixed', () => {
  const {foundation, mockAdapter} = setupTest();
  const flexibleExpansionRatio = 1;

  foundation.changeElementStylesDefaultBehavior_(flexibleExpansionRatio);
